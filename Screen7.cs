	...
	void ShowPetsNew()
	{
		...
		for(int i = 0; i<GlobalScript.petDatas.Count; i++)
		{
			GUI.BeginGroup(new Rect(i*130, 0, 130, 171));
				...
				//Pet mana
				Stretch(1, new Rect(18, 125, 94, 12), GlobalScript.petDatas[i].mana, GlobalScript.petDatas[i].maxMana, false);
				//Pet Blood
				GlobalScript.drawStretch(2, new Rect(18, 142, 94, 12), GlobalScript.petDatas[i].food, GlobalScript.petDatas[i].maxFood, false);
			GUI.EndGroup();
		}
		GUI.EndGroup();
	}
