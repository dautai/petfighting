...
public class Screen6 : MonoBehaviour {
	...
	void OnGUI()
	{
		...
		GUI.BeginGroup(new Rect((Screen.width)/2-160, (Screen.height)/2-85, 320, 170));
			...
			GS.brightness = GUI.HorizontalSlider(new Rect(160, 0, 160, 30), GS.brightness, 0f, 100f);
			if(GUI.Button(new Rect(160, 40, 160, 30), GS.resolution))
			{
				if(selecting)
				selecting = false;
			else
			...
			GS.volumn = GUI.HorizontalSlider(new Rect(160, 80, 160, 30), GS.volumn, 0f, 100f);
	 ...
	 void DoMyWindow(int windowID) {
		...
		GUI.BeginGroup(new Rect(0, 0, 160, 35*n));
			for(int i=0; i<n; i++)
			{
				...
				{
					selectionGridInt = i;
					GS.resolution = selectionStrings[selectionGridInt];
					selecting = false;
				}
			}
			GUI.EndGroup();
